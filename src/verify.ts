...
export class Verify {
    ...
    public async modalSubmit(interaction: Discord.ModalSubmitInteraction) {
        ...
        const success = await this.sendEmail(email, this.code, firstName, interaction.guild.name);
    ...
    public codeButtonSubmit(interaction: Discord.ButtonInteraction) {
        const modal = new Discord.ModalBuilder()
    ...
    public async codeModalSubmit(interaction: Discord.ModalSubmitInteraction) {
        const codeInput: string = interaction.fields.getTextInputValue("codeInput");
        if (codeInput === this.code) {
            // verify guild member
            const member = interaction.member as Discord.GuildMember;
            const verified = await this.verifyUser(member);
            if (verified) {
                let embed: Discord.EmbedBuilder = new Discord.EmbedBuilder()
                    .setColor(0x00FF00)
                    .setTitle(`Success: `)
     ...
     */
    private async sendEmail(to: string, code: string, name: string, serverName: string): Promise<boolean> {
        try {
            const body = `<html>
            <p>Hello ${name},</p>
    ...
    private async verifyUser(member: Discord.GuildMember): Promise<boolean> {
        try {
